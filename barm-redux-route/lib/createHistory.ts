...
export const createHistory = (store: any): IHistory => {
  ...
  /** 根据浏览器访问的URL初始化路径 */
  function dispatchInitHistory(initUrl: string, keepHistory?: boolean, hashSpace = '#') {
    ...
    // 若当前url等于默认url，仅切换到默认url
    if (path === '/' || path === initUrl) {
      dispatchHistoryPush(initUrl, queryString.parse(search));
    } else {
      // 若当前url不等于默认url，先渲染默认url， 再切换到当前url
      dispatchHistoryPush(initUrl);
      setTimeout(() => {
